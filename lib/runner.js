...
app.on('ready', function() {
  ...
  parent.on('type', function (type) {
    var start = Date.now()
    type.split('').forEach(function (ch) {
  ...
  parent.on('html', function(path) {
    win.webContents.savePage(path, 'HTMLOnly', function(err) {
      parent.emit('html', arguments);
    });
  });
